...
Writes out/cooccur_{B1,B2}_K{15,200}_final.csv and out/c1_clades{SUF}.json.
...
SUF=_os.environ.get("RUN_SUF","")
TREE  = _os.environ.get("RUN_TREE",_support("mergedtree_dttpq_v8.npz", "DTT_MERGED_TREE_NPZ", 'Generate it with: python3 tools/make_merged_tree_npz.py (derived from support_data/merged_full_placed.nwk).'))
...
json.dump({"universe": universe, "n_types": T, "min_cells": MIN_CELLS,
          ...
          open(f"{HERE}/out/c1_clades{SUF}.json", "w"), indent=1)
print(f"\nwrote out/cooccur_{{B1,B2}}_K{{{','.join(map(str,KS))}}}_final.csv, out/c1_clades{SUF}.json")

...
for bl, blname in BLASTS:
    ...
    log(f"\n=== blastomere {blname} ({bl}): {int(pmask.sum()):,} sibling pairs, "
        f"{len(pool_ix[bl]):,} annotated tips in permutation pool ===")
...
for p in range(NPERM):
    for bl, _ in BLASTS:
        labp[pool_ix[bl]] = rng.permutation(lab0[pool_ix[bl]])
    pm_all = is_pm_t[labp]
...
np.savez_compressed(f"{HERE}/out/h2_raw.npz",
...
for am, al, dm in cells:
    ...
    with open(f"{HERE}/out/h2_pairings_{tag}.csv", "w", newline="") as fh:
        w = csv.writer(fh)
...
with open(f"{HERE}/out/h2_sweep.csv", "w", newline="") as fh:

...
Outputs: out/h2_sweep.csv (grid summary), out/h2_pairings_<tag>.csv (per-pairing detail for
every grid cell), out/h2_raw.npz (obs/exp/var matrices).  Deterministic given NPERM/SEED.
"""
import gzip
...
NULL_MODE = os.environ.get("NULL_MODE","global")
d = np.load(f"{HERE}/out/h1_pairs{SUF}.npz", allow_pickle=True)
...
# ---- per-leaf permutation stratum, selected by NULL_MODE -------------------------------
#   global : one pool per blastomere (the manuscript's null)
#   traj   : within each major trajectory
#   clone  : within each founder clone dating to E6.0
STRAT = np.zeros(len(leaf_ct), np.int64)
if NULL_MODE == "traj":
    import csv as _csv
    _tj = {}
    ...
    _tn = [str(t) for t in d["types"]]
    _u = sorted({_tj.get(t, "") for t in _tn})
    _map = np.array([_u.index(_tj.get(t, "")) for t in _tn], np.int64)
    STRAT = np.where(leaf_ct >= 0, _map[np.clip(leaf_ct, 0, None)], -1)
    ...
    _k = [[] for _ in range(_N)]
    for _v in range(1, _N): _k[_par[_v]].append(_v)
    _cl = np.full(_N, -1, np.int64)
    _cross = [v for v in range(1, _N) if _tm[_par[v]] < 6.0 <= _tm[v]]
    for _i, _v in enumerate(_cross):
        _st = [_v]
        while _st:
            _u2 = _st.pop(); _cl[_u2] = _i; _st.extend(_k[_u2])
    STRAT = _cl[np.flatnonzero(_il)]
    log(f"null=clone: {len(_cross)} founder clones at E6.0")
...
for bl, blname in BLASTS:
    pmask = pair_blast == bl
    _p = np.flatnonzero((leaf_blast == bl) & (leaf_ct >= 0))
    if NULL_MODE == "global":
        pool_ix[bl] = [_p]
    else:
        _g = {}
        for _v in _p: _g.setdefault(int(STRAT[_v]), []).append(_v)
        pool_ix[bl] = [np.array(v) for v in _g.values()]
    log(f"\n=== blastomere {blname} ({bl}): {int(pmask.sum()):,} sibling pairs, "
        f"{sum(len(x) for x in pool_ix[bl]):,} tips in {len(pool_ix[bl])} strata "
        f"[null={NULL_MODE}] ===")
...
np.savez_compressed(f"{HERE}/out/h2_raw{SUF}_{NULL_MODE}.npz",
...
for am, al, dm in cells:
    ...
    with open(f"{HERE}/out/h2_pairings{SUF}_{NULL_MODE}_{tag}.csv", "w", newline="") as fh:
        w = csv.writer(fh)
...
with open(f"{HERE}/out/h2_sweep{SUF}_{NULL_MODE}.csv", "w", newline="") as fh:
...
print(f"\nwrote out/h2_sweep.csv, out/h2_pairings_*.csv, out/h2_raw.npz [{time.time()-t0:.0f}s]")

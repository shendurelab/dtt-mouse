...
for seed in range(10):
    ...
    pca_impute_full.to_csv(f"{work_path}/pca_impute_seed_{seed}.tsv", sep="\t")
...
write.table(pd_internal_x, paste0(work_path, "/internal_assign_cells_200_seed_", seed, ".txt"), row.names=F, col.names=F, sep="\t", quote=F)
...
ggsave("boxplot_assign_accuracy_by_day.pdf", p, width = 6, height = 4)
...
p = ggplot(df_plot, aes(log2_cell_num, pct)) + 
    ...
    theme_classic(base_size = 10) +
    theme(legend.position = "none")
ggsave("boxplot_assign_accuracy_by_celltype.pdf", p, width = 5, height = 5)

...
adata.write(f"{work_path}/adata_integration_early.h5ad", compression="gzip")

adata.obs.to_csv(f"{work_path}/adata_integration_early.obs.csv")
pd.DataFrame(adata.obsm['X_pca']).to_csv(f"{work_path}/adata_integration_early.pca.csv")
...
pd_new = readRDS(paste0(work_path, "/adata_integration.obs.rds"))

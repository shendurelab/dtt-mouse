...
if os.environ.get("DROP_BLOOD") == "1":
    _BLOOD = {"White_blood_cells","Definitive_erythroid","Primitive_erythroid","Megakaryocytes",
    ...
    _n0 = len(universe)
    universe = [t for t in universe if _tj.get(t, "") not in _BLOOD]
    print(f"DROP_BLOOD: {_n0} -> {len(universe)} cell types "
          f"({_n0-len(universe)} haematopoietic removed)", file=sys.stderr)
...
np.savez_compressed(f"{HERE}/out/c7_timesweep{TAG}.npz", types=np.array(universe, object),
                    TIMES=np.array(TIMES), **store)
json.dump(meta, open(f"{HERE}/out/c7_timesweep_meta{TAG}.json", "w"), indent=1)
...
with open(f"{HERE}/out/c7_divergence_ages{TAG}.csv", "w", newline="") as fh:
    w = csv.writer(fh)
...
fig.tight_layout(); fig.savefig(f"{HERE}/out/c7_carpet{TAG}.png", dpi=145, bbox_inches="tight")
...
fig.tight_layout(); fig.savefig(f"{HERE}/out/c7_divmatrix{TAG}.png", dpi=145, bbox_inches="tight")
...
fig.tight_layout(); fig.savefig(f"{HERE}/out/c7_ab_divage{TAG}.png", dpi=150)
